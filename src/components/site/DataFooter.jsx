...
export default function DataFooter({ onNavigate }) {
  ...
  const legalLinks = [
  { label: t("footer.privacy"), path: buildPath("privacy", language) }];
      ...
      <div className="flex flex-col lg:flex-row justify-between gap-12 lg:gap-0">
        ...
        <div className="flex gap-12">
          ...
          <div className="space-y-3">
            ...
            {legalLinks.map((link) =>
            <button key={link.path} onClick={() => navigate(link.path)} className="block text-left font-heading text-xs text-titanium/40 hover:text-brand-green transition-colors">
                {link.label}
              </button>
            )}
          </div>
